...
function Map(props) {
  ...
  useEffect(() => { // Hook d'effet pour effectuer des appels API au chargement du composant
    const fetchData = async () => { // fonction asynchrone pour effectuer l'appel API
      try {
        const response = await axios.get(`${baseUrl}/plant`); // fait un appel GET à l'URL /plant
        const plantsData = response.data.Plants; // extrait les données "Plants" de la réponse
    ...
    <>
      ...
      <div className="map-wrap">
        <div className="card card-map">
          <MapContainer center={positionLille} zoom={6} scrollWheelZoom={false}> // affiche la carte centrée sur la position de Lille avec un zoom de 6
            <TileLayer
